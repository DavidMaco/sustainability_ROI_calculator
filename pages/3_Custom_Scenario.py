...
# Store session-scoped assumptions
assumptions = EconomicAssumptions(carbon_tax=carbon_tax, water_cost=water_cost, waste_cost=waste_cost)

# ─── Input form ──────────────────────────────────────────────────────
st.subheader("Enter Your Annual Procurement Volumes")
st.caption("Set quantity to 0 for materials you don't use.")
...
if st.button("Calculate ROI", type="primary", disabled=len(custom_mix) == 0):
    ...
    payback_str = f"{result.payback_years:.2f} yrs" if result.payback_years else "Immediate"
    c6.metric("Payback Period", payback_str)

    st.markdown("#### Savings Breakdown")
    breakdown = pd.DataFrame(
        {
            "Category": ["Carbon Tax Avoidance", "Water Cost Savings", "Waste Disposal Savings"],
            "Amount (₦)": [
                result.carbon_tax_savings_ngn,
                result.water_cost_savings_ngn,
                result.waste_disposal_savings_ngn,
            ],
        }
    )
    st.dataframe(
